...
\title{Adaptive Hierarchial Sliding Mode Control with Input Saturation for Attitude Regulation of Multi-satellite Tethered System%\thanks{Grants or other notes
...
\end{figure}
The schematic representation of the three-satellite inline array is present in Figure~\ref{Figure:3satellite}, and the simplified Euler's rotational equations of the motion dynamics follows
\begin{align}
\begin{bmatrix}
J_1 &0  &0\\
0   &J_2&0\\
...
\end{align}
where $\bar{sat}()$ means the saturation characteristics. According to the zero tether length rate assumption $\dot l=0$, the preceding equations coincides with Eq.~(\ref{eq:3bodyreduce}). Attend to the problem of the reduced three-satellite inline tethered formation, and the following lemma is proposed to demonstrate the stability of the coupled attitude dynamics shown in Eq.~(\ref{eq:3bodymodel}).
\begin{lemma}
